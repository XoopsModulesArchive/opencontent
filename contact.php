...
if ($submit) {
    ...
    $result = $xoopsDB->query('SELECT title, email FROM ' . $xoopsDB->prefix('opencontent_links') . " WHERE lid = '$id'");
...
OpenTable();
 ...
 $result = $xoopsDB->query('SELECT title, email FROM ' . $xoopsDB->prefix('opencontent_links') . " WHERE lid = '$lid'");
while (list($title, $email) = $xoopsDB->fetchRow($result)) {
    echo '<script>
          function verify() {
