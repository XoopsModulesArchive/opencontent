...
function opencontent()
{
    ...
    $result = $xoopsDB->query('select count(*) from ' . $xoopsDB->prefix('opencontent_broken') . '');
    ...
    $result2 = $xoopsDB->query('select count(*) from ' . $xoopsDB->prefix('opencontent_mod') . '');
    ...
    $result3 = $xoopsDB->query('select count(*) from ' . $xoopsDB->prefix('opencontent_links') . ' where status=0');
    ...
    $result = $xoopsDB->query('select count(*) from ' . $xoopsDB->prefix('opencontent_links') . ' where status>0');
...
function listNewLinks()
{
    ...
    $linkimg_array = XoopsLists::getImgListAsArray(XOOPS_ROOT_PATH . '/modules/opencontent/images/shots/');

    $result = $xoopsDB->query('select lid, cid, title, address, address2, city, state, zip, country, phone, fax, email, url, logourl, submitter, premium from ' . $xoopsDB->prefix('opencontent_links') . ' where status=0 order by date DESC');
    ...
    if ($numrows > 0) {
        while (list($lid, $cid, $title, $address, $address2, $city, $state, $zip, $country, $phone, $fax, $email, $url, $logourl, $submitterid, $premium) = $xoopsDB->fetchRow($result)) {
            $result2 = $xoopsDB->query('select description from ' . $xoopsDB->prefix('opencontent_text') . " where lid=$lid");
            ...
            $shotdir = '<b>' . XOOPS_URL . '/modules/opencontent/images/shots/</b>';
            ...
            echo '<option value="1">1 Degno di nota</option>';

            echo '<option value="2">2 Importante</option>';

            echo '<option value="3">3 Veramente Importante</option>';

            echo '<option value="4">4 Segnalazione Bussines</option>';

            echo '<option value="5">5 Segralazione Top Bussines</option>';
...
function linksConfigMenu()
{
    ...
    $result = $xoopsDB->query('select count(*) from ' . $xoopsDB->prefix('opencontent_cat') . '');
    ...
    $result2 = $xoopsDB->query('select count(*) from ' . $xoopsDB->prefix('opencontent_links') . '');
    ...
    . '<tr class="odd"><td>';
        ...
        echo "<input type=hidden name=fct value=opencontent>\n";
...
function modLink()
{
    ...
    $result = $xoopsDB->query('select cid, title, address, address2, city, state, zip, country, phone, fax, email, url, logourl, premium from ' . $xoopsDB->prefix('opencontent_links') . " where lid=$lid") or $eh::show('0013');

    echo '<h4>' . _MD_MODLINK . '</h4><br>';

    [$cid, $title, $address, $address2, $city, $state, $zip, $country, $phone, $fax, $email, $url, $logourl, $premium] = $xoopsDB->fetchRow($result);
    ...
    $result5 = $xoopsDB->query('SELECT count(*) FROM ' . $xoopsDB->prefix('opencontent_votedata') . " WHERE lid = $lid");
    ...
    $result5 = $xoopsDB->query('SELECT ratingid, ratinguser, rating, ratinghostname, ratingtimestamp FROM ' . $xoopsDB->prefix('opencontent_votedata') . " WHERE lid = $lid AND ratinguser >0 ORDER BY ratingtimestamp DESC");
    ...
    while (list($ratingid, $ratinguser, $rating, $ratinghostname, $ratingtimestamp) = $xoopsDB->fetchRow($result5)) {
        ...
        $result2 = $xoopsDB->query('SELECT rating FROM ' . $xoopsDB->prefix('opencontent_votedata') . " WHERE ratinguser = '$ratinguser'");
    ...
    $result5 = $xoopsDB->query('SELECT ratingid, rating, ratinghostname, ratingtimestamp FROM ' . $xoopsDB->prefix('opencontent_votedata') . " WHERE lid = $lid AND ratinguser = 0 ORDER BY ratingtimestamp DESC");
...
function delVote()
{
    ...
    $sql = sprintf('DELETE FROM %s WHERE ratingid = %u', $xoopsDB->prefix('opencontent_votedata'), $rid);
...
function listBrokenLinks()
{
    ...
    $result = $xoopsDB->query('select * from ' . $xoopsDB->prefix('opencontent_broken') . ' group by lid order by reportid DESC');
    ...
    if (0 == $totalbrokenlinks) {
        ...
        while (list($reportid, $lid, $sender, $ip) = $xoopsDB->fetchRow($result)) {
            $result2 = $xoopsDB->query('select title, url, submitter from ' . $xoopsDB->prefix('opencontent_links') . " where lid=$lid");
...
function delBrokenLinks()
{
    ...
    $sql = sprintf('DELETE FROM %s WHERE lid = %u', $xoopsDB->prefix('opencontent_broken'), $lid);

    $xoopsDB->query($sql) or $eh::show('0013');

    $sql = sprintf('DELETE FROM %s WHERE lid = %u', $xoopsDB->prefix('opencontent_links'), $lid);
...
function ignoreBrokenLinks()
{
    ...
    $sql = sprintf('DELETE FROM %s WHERE lid = %u', $xoopsDB->prefix('opencontent_broken'), $_GET['lid']);
...
function listModReq()
{
    ...
    $result = $xoopsDB->query('select * from ' . $xoopsDB->prefix('opencontent_mod') . ' order by requestid');
    ...
    if ($totalmodrequests > 0) {
        ...
        while (list($requestid, $lid, $cid, $title, $address, $address2, $city, $state, $zip, $country, $phone, $fax, $email, $url, $logourl, $premium, $description, $modifysubmitter) = $xoopsDB->fetchRow($result)) {
            ...
            $result2 = $xoopsDB->query('select cid, title, address, address2, city, state, zip, country, phone, fax, email, url, logourl, submitter from ' . $xoopsDB->prefix('opencontent_links') . " where lid=$lid");

            [$origcid, $origtitle, $origaddress, $origaddress2, $origcity, $origstate, $origzip, $origcountry, $origphone, $origfax, $origemail, $origurl, $origlogourl, $ownerid] = $xoopsDB->fetchRow($result2);
            ...
            if ($xoopsModuleConfig['useshots'] && !empty($origlogourl)) {
                echo '<img src="' . XOOPS_URL . '/modules/opencontent/images/shots/' . $origlogourl . '" width="' . $xoopsModuleConfig['shotwidth'] . '">';
            } else {
                echo ' ';
            }
            ...
            if (1 == $xoopsModuleConfig['useshots'] && !empty($logourl)) {
                echo '<img src="' . XOOPS_URL . '/modules/opencontent/images/shots/' . $logourl . '" width="' . $xoopsModuleConfig['shotwidth'] . '" alt="/">';
            } else {
                echo ' ';
            }
...
function changeModReq()
{
    ...
    $query = 'select requestid, lid, cid, title, address, address2, city, state, zip, country, phone, fax, email, url, logourl, premium, description from ' . $xoopsDB->prefix('opencontent_mod') . ' where requestid=' . $requestid . '';
    ...
    while (list($requestid, $lid, $cid, $title, $address, $address2, $city, $state, $zip, $country, $phone, $fax, $email, $url, $logourl, $premium, $description) = $xoopsDB->fetchRow($result)) {
        ...
        $sql = sprintf("UPDATE %s SET cid = %u, title = '%s', address = '%s', address2 = '%s', city = '%s', state = '%s', zip = '%s', country = '%s', phone = '%s', fax = '%s', email = '%s', url = '%s', logourl = '%s', status = %u, date = %u, premium = %u WHERE lid = %u", $xoopsDB->prefix('opencontent_links'), $cid, $title, $address, $address2, $city, $state, $zip, $country, $phone, $fax, $email, $url, $logourl, 2, time(), $premium, $lid);

        $xoopsDB->query($sql) or $eh::show('0013');

        $sql = sprintf("UPDATE %s SET description = '%s' WHERE lid = %u", $xoopsDB->prefix('opencontent_text'), $description, $lid);

        $xoopsDB->query($sql) or $eh::show('0013');

        $sql = sprintf('DELETE FROM %s WHERE requestid = %u', $xoopsDB->prefix('opencontent_mod'), $requestid);
...
function ignoreModReq()
{
    ...
    $sql = sprintf('DELETE FROM %s WHERE requestid = %u', $xoopsDB->prefix('opencontent_mod'), $_GET['requestid']);
...
function modLinkS()
{
    ...
    $xoopsDB->query('update ' . $xoopsDB->prefix('opencontent_links') . " set cid='$cid', title='$title', address='$address', address2='$address2', city='$city', state='$state', zip='$zip', country='$country', phone='$phone', fax='$fax', email='$email', url='$url', logourl='$logourl', status=2, date=" . time() . ", premium='$premium' where lid=" . $_POST['lid'] . '') or $eh::show('0013');

    $xoopsDB->query('update ' . $xoopsDB->prefix('opencontent_text') . " set description='$description' where lid=" . $_POST['lid'] . '') or $eh::show('0013');
...
function delLink()
{
    ...
    $sql = sprintf('DELETE FROM %s WHERE lid = %u', $xoopsDB->prefix('opencontent_links'), $_GET['lid']);

    $xoopsDB->query($sql) or $eh::show('0013');

    $sql = sprintf('DELETE FROM %s WHERE lid = %u', $xoopsDB->prefix('opencontent_text'), $_GET['lid']);

    $xoopsDB->query($sql) or $eh::show('0013');

    $sql = sprintf('DELETE FROM %s WHERE lid = %u', $xoopsDB->prefix('opencontent_votedata'), $_GET['lid']);
...
function modCat()
{
    ...
    $result = $xoopsDB->query('select pid, title, imgurl from ' . $xoopsDB->prefix('opencontent_cat') . " where cid=$cid");
...
function modCatS()
{
    ...
    $xoopsDB->query('update ' . $xoopsDB->prefix('opencontent_cat') . " set pid=$pid, title='$title', imgurl='$imgurl' where cid=$cid") or $eh::show('0013');
...
function delCat()
{
    ...
    if (1 == $ok) {
        ...
        for ($i = 0; $i < $dcount; $i++) {
            ...
            $result = $xoopsDB->query('select lid from ' . $xoopsDB->prefix('opencontent_links') . ' where cid=' . $arr[$i] . '') or $eh::show('0013');

            //now for each link, delete the text data and vote ata associated with the link

            while (list($lid) = $xoopsDB->fetchRow($result)) {
                $sql = sprintf('DELETE FROM %s WHERE lid = %u', $xoopsDB->prefix('opencontent_text'), $lid);

                $xoopsDB->query($sql) or $eh::show('0013');

                $sql = sprintf('DELETE FROM %s WHERE lid = %u', $xoopsDB->prefix('opencontent_votedata'), $lid);
            ...
            $sql = sprintf('DELETE FROM %s WHERE cid = %u', $xoopsDB->prefix('opencontent_cat'), $arr[$i]);
        ...
        $result = $xoopsDB->query('select lid from ' . $xoopsDB->prefix('opencontent_links') . ' where cid=' . $cid . '') or $eh::show('0013');
        ...
        $sql = sprintf('DELETE FROM %s WHERE cid = %u', $xoopsDB->prefix('opencontent_cat'), $cid);
...
function addCat()
{
    ...
    $newid = $xoopsDB->genId($xoopsDB->prefix('opencontent_cat') . '_cid_seq');

    $sql = sprintf("INSERT INTO %s (cid, pid, title, imgurl) VALUES (%u, %u, '%s', '%s')", $xoopsDB->prefix('opencontent_cat'), $newid, $pid, $title, $imgurl);
...
function addLink()
{
    ...
    $result = $xoopsDB->query('select count(*) from ' . $xoopsDB->prefix('opencontent_links') . " where title='$title'");
    ...
    $newid = $xoopsDB->genId($xoopsDB->prefix('opencontent_links') . '_lid_seq');

    $sql = sprintf("INSERT INTO %s (lid, cid, title, address, address2, city, state, zip, country, phone, fax, email, url, logourl, submitter, status, date, hits, rating, votes, comments, premium) VALUES (%u, %u, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %u, %u, %u, %u, %u, %u, %u, %u)", $xoopsDB->prefix('opencontent_links'), $newid, $cid, $title, $address, $address2, $city, $state, $zip, $country, $phone, $fax, $email, $url, $logourl, $submitter, 1, time(), 0, 0, 0, 0, $premium);
    ...
    $sql = sprintf("INSERT INTO %s (lid, description) VALUES (%u, '%s')", $xoopsDB->prefix('opencontent_text'), $newid, $description);
    ...
    $sql = 'SELECT title FROM ' . $xoopsDB->prefix('opencontent_cat') . ' WHERE cid=' . $cid;
...
function approve()
{
    ...
    $address2 = $myts->addSlashes($address2);

    $city = $myts->addSlashes($city);

    $state = $myts->addSlashes($state);

    $zip = $myts->addSlashes($zip);

    $country = $myts->addSlashes($country);

    $phone = $myts->addSlashes($phone);

    $fax = $myts->addSlashes($fax);
    ...
    $query = 'update ' . $xoopsDB->prefix('opencontent_links') . " set cid='$cid', title='$title', address='$address', address2='$address2', city='$city', state='$state', zip='$zip', country='$country', phone='$phone', fax='$fax', email='$email', url='$url', logourl='$logourl', premium='$premium', status=1, date=" . time() . ' where lid=' . $lid . '';

    $xoopsDB->query($query) or $eh::show('0013');

    $query = 'update ' . $xoopsDB->prefix('opencontent_text') . " set description='$description' where lid=" . $lid . '';
