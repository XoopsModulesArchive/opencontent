...
if (!empty($_POST['submit'])) {
    ...
    if (0 != $sender) {
        ...
        $result = $xoopsDB->query('SELECT COUNT(*) FROM ' . $xoopsDB->prefix('opencontent_broken') . ' WHERE lid=' . $lid . ' AND sender=' . $sender . '');
        ...
        $result = $xoopsDB->query('SELECT COUNT(*) FROM ' . $xoopsDB->prefix('opencontent_broken') . " WHERE lid=$lid AND ip = '$ip'");
    ...
    $newid = $xoopsDB->genId($xoopsDB->prefix('opencontent_broken') . '_reportid_seq');

    $sql = sprintf("INSERT INTO %s (reportid, lid, sender, ip) VALUES (%u, %u, %u, '%s')", $xoopsDB->prefix('opencontent_broken'), $newid, $lid, $sender, $ip);
    ...
    $GLOBALS['xoopsOption']['template_main'] = 'opencontent_brokenlink.html';
