...
function newlinkgraphic($time, $status)
{
    ...
    if ($startdate < $time) {
        if (1 == $status) {
            $new = '&nbsp;<img src="' . XOOPS_URL . '/modules/opencontent/images/newred.gif" alt="' . _MD_NEWTHISWEEK . '">';
        } elseif (2 == $status) {
            $new = '&nbsp;<img src="' . XOOPS_URL . '/modules/opencontent/images/update.gif" alt="' . _MD_UPTHISWEEK . '">';
        }
    }
...
function popgraphic($hits)
{
    ...
    if ($hits >= $xoopsModuleConfig['popular']) {
        return '&nbsp;<img src="' . XOOPS_URL . '/modules/opencontent/images/pop.gif" alt="' . _MD_POPULAR . '">';
    }
...
function updaterating($sel_id)
{
    ...
    $query = 'select rating FROM ' . $xoopsDB->prefix('opencontent_votedata') . ' WHERE lid = ' . $sel_id . '';
    ...
    $query = 'UPDATE ' . $xoopsDB->prefix('opencontent_links') . " SET rating=$finalrating, votes=$votesDB WHERE lid = $sel_id";
...
function getTotalItems($sel_id, $status = '')
{
    ...
    $query = 'select count(*) from ' . $xoopsDB->prefix('opencontent_links') . ' where cid=' . $sel_id . '';
    ...
    for ($i = 0; $i < $size; $i++) {
        $query2 = 'select count(*) from ' . $xoopsDB->prefix('opencontent_links') . ' where cid=' . $arr[$i] . '';
