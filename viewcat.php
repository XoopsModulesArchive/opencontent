...
$cid = (int)$_GET['cid'];
$GLOBALS['xoopsOption']['template_main'] = 'opencontent_viewcat.html';
...
if (count($arr) > 0) {
    ...
    foreach ($arr as $ele) {
        ...
        foreach ($sub_arr as $sub_ele) {
            ...
            $infercategories .= '<a href="' . XOOPS_URL . '/modules/opencontent/viewcat.php?cid=' . $sub_ele['cid'] . '">' . $chtitle . '</a>';
...
$fullcountresult = $xoopsDB->query('select count(*) from ' . $xoopsDB->prefix('opencontent_links') . " where cid=$cid and status>0");
...
if ($numrows > 0) {
    ...
    while (list($lid, $cid, $ltitle, $address, $address2, $city, $state, $zip, $country, $phone, $fax, $email, $url, $logourl, $status, $time, $hits, $rating, $votes, $comments, $premium, $description) = $xoopsDB->fetchRow($result)) {
        //			$result2 = $xoopsDB->query("select lid, cid, title, address, address2, city, state, zip, country, phone, fax, email, url, logourl, submitter, status, date, hits, rating, votes, comments from ".$xoopsDB->prefix("opencontent_links")." where cid=$cid and premium==1");
        ...
        $xoopsTpl->append('links', ['id' => $lid, 'cid' => $cid, 'url' => $url, 'rating' => number_format($rating, 2), 'title' => htmlspecialchars($ltitle, ENT_QUOTES | ENT_HTML5) . $new . $pop, 'address' => htmlspecialchars($address, ENT_QUOTES | ENT_HTML5), 'address2' => htmlspecialchars(
            ...
            $phone,
            ENT_QUOTES | ENT_HTML5
        ), 'fax'                         => htmlspecialchars($fax, ENT_QUOTES | ENT_HTML5), 'email' => htmlspecialchars($email, ENT_QUOTES | ENT_HTML5), 'category' => $path, 'logourl' => htmlspecialchars($logourl, ENT_QUOTES | ENT_HTML5), 'updated' => formatTimestamp($time, 'm'), 'description' => $myts->displayTarea($description, 0), 'adminlink' => $adminlink, 'hits' => $hits, 'comments' => $comments, 'premium' => $premium, 'votes' => $votestring, 'mail_subject' => rawurlencode(sprintf(_MD_INTRESTLINK, $xoopsConfig['sitename'])), 'mail_body' => rawurlencode(sprintf(_MD_INTLINKFOUND, $xoopsConfig['sitename']) . ':  ' . XOOPS_URL . '/modules/opencontent/singlelink.php?cid=' . $cid . '&lid=' . $lid)]);
    }
